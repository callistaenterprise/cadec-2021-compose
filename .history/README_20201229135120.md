...
> 5. Data trickles down from the top level composable.
  ...
  - Events
When the user interacts with the UI, through say an onClick.  These will notify the app logic which will mutate the state, which is then trickled down by calling the composable with fresh arguments. This is known as recomposition.
